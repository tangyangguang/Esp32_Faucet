...
| `WaterSensorManager` | 1s 采样、ADC 故障降级、NTC 断短路识别、输入电压校准、TDS 自动量程、实时快照和出水摘要聚合 |
...
- 温度校准详情页输入温度计读数后保存偏移；NTC 的 R25、B 值、上拉标称阻值在系统设置中配置，探头断路/短路直接产生无效值。TDS 校准详情页以 1-5 个校准点作为统一模型，稳定窗口使用中位数形成校准点，未接导致的 0 不推断为断线；已保存点只允许修改参考 ppm，原始 ppm 和采样电压保持不变并自动重算。输入电压校准同样统一使用 1-5 点：一点按零偏移比例换算，两点以上线性回归，每点保留 ADC 原始窗口和理论/实际对应关系；采集和实时显示统一使用最近 5 次 ADC 原始值的中位数，波动超限时不允许保存；只允许修改万用表实际值，修改后重新拟合。
