...
- 出水记录已接入 LittleFS 文件环形存储：通过 Esp32Base Fs 按偏移读写 API 写入二进制定长记录，文件不可用时保留 RAM 环形降级写入。当前 4MB Flash 分区预算下工程容量固定为 20,000 条，达到容量边界后覆盖最旧记录。记录包含目标值、原始脉冲数、过滤脉冲数和当时计量方案 ID，不保存完整计量参数快照。记录校准元数据使用独立文件保存实测量、校准类型、参数变化和重校次数，不改写出水记录本体。文件启动时只创建头部，记录按需追加，满容量后再环形覆盖，避免裸板首次启动预分配大文件触发 watchdog。
...
- Web 写配置类 API 已通过 Esp32BaseWeb 当前请求方法能力接入 POST 保存：支持配置、单个预设保存、记录校准、脉冲明细保存/删除、计量方案写入和滤芯重置；空闲状态下保存后立即热更新或写入对应业务存储，出水/确认/暂停期间拒绝修改并返回 busy。
