...
6. Web 页面和 API
   ...
   - Web 记录页允许基于最新记录输入量杯实际水量校准，校准页保留手动录入系数，不允许打开电磁阀。
...
- 出水日志已接入 LittleFS 文件环形存储：通过 Esp32Base Fs 按偏移读写 API 实现 20000 条目标容量的二进制定长日志，文件不可用时保留 RAM 环形降级写入。记录包含目标值、原始脉冲数、过滤脉冲数和当时流量系数。文件启动时只创建头部，记录按需追加，满容量后再环形覆盖，避免裸板首次启动预分配大文件触发 watchdog。
...
- Web 页面已从安全占位升级为轻量可用页面：状态、配置、预设、日志、统计、滤芯、校准页面已接入；首页用卡片展示启用滤芯寿命进度；滤芯列表页只显示查看、状态、设置入口和重置，单个滤芯配置放在独立编辑页；记录页支持分页按钮和每页条数下拉，选项为 20、30、50、100、200，默认 20，并显示原始脉冲及最新记录校准入口；校准页保留手动配置每升信号数和本地候选容量；页面仍不提供任何远程出水控制入口。
