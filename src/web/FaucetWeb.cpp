...
void sendCalibrationParameterPanels() {
    ...
    for (std::size_t i = 0; i < count; ++i) {
        ...
        sendFmt("<a class='btn-link' href='/faucet/calibration?scheme=%lu'>编辑</a>",
                static_cast<unsigned long>(scheme.id));
        if (scheme.id != activeId) {
            sendFmt("<form method='post' action='/faucet/calibration' onsubmit=\"return confirm('确认切换当前计量方案？')&&once(this)\"><input type='hidden' name='action' value='enable_metering_scheme'><input type='hidden' name='id' value='%lu'><input class='primary' type='submit' value='%s'></form>",
                    static_cast<unsigned long>(scheme.id),
...
void sendMachineStatusCard(const AppSnapshot& snapshot, bool screenOn) {
    ...
    sendFmt("</span></strong></div><div id='machineRunningNoteRow' class='machine-running-note-row'%s>"
            "<p id='machineStatusNote' class='machine-running-note'>%s</p></div><div class='machine-context'>",
...
void sendHomeAutoRefreshScript() {
    Esp32BaseWeb::sendChunk("<script>"
                            ...
                            "faucetToggle('machineRunningNoteRow',s.state==='running');"
