...
- Web 页面已从安全占位升级为轻量可用页面：状态、预设、记录、统计、滤芯页面已接入；首页用卡片展示启用滤芯寿命进度；滤芯列表页只显示查看、状态、设置入口和重置，单个滤芯配置放在独立编辑页；记录页支持分页按钮和每页条数下拉，选项为 20、30、50、100、200，默认 20，并显示原始脉冲及最新记录校准入口；时间范围筛选尚未接入，是 records 首版剩余必做项；页面仍不提供任何远程出水控制入口。
