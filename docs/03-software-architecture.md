...
- 流量计计量方案使用独立持久化域：保存当前启用方案 ID、计量方案生成结果和动态方案记录。系统内置 `YF-S201 默认计量方案` 作为首次启动兜底参数，启动脉冲数 8P、启动水量 36ml、稳态 225P/L。运行时只读取当前方案的启动脉冲数、启动水量、稳态 P/L；生成结果只能保存为新方案，保存后不自动启用，启用必须是单独动作。已经被出水记录使用过的方案不能删除，只能停用；从未使用过、非当前且不是最后一套有效方案的方案可以删除。
...
| POST | `/api/faucet/records` | 通过 `action` 执行记录校准、脉冲明细保存/删除、分段样本生成和计量方案 create/edit/enable/disable/delete；危险 POST 必须通过同源校验；出水确认、运行和暂停期间返回 busy |
