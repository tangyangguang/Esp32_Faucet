...
void test_web_page_source_contains_expected_ui_improvements() {
    ...
    TEST_ASSERT_NOT_NULL(std::strstr(buffer, "序号\\telapsedUs\\t间隔Us\\t有效累计\\n"));
    ...
    TEST_ASSERT_NOT_NULL(std::strstr(buffer, "<table class='raw-trace-table'><tr><th>序号</th><th>elapsedUs</th><th>间隔Us</th><th>有效累计</th></tr>"));
    TEST_ASSERT_NOT_NULL(std::strstr(buffer, "<tr><td>%lu</td><td>%lu</td><td>%lu</td><td>%lu</td></tr>"));
    ...
    TEST_ASSERT_NULL(std::strstr(buffer, "value='保存到设备'"));
    TEST_ASSERT_NOT_NULL(std::strstr(buffer, "已保存到设备"));
    TEST_ASSERT_NULL(std::strstr(buffer, "确认将这条脉冲明细保存到设备存储？"));
    ...
    TEST_ASSERT_NOT_NULL(std::strstr(buffer, "这里展示有脉冲明细的数据。RAM 临时样本重启会丢失；只有已保存到设备、容量已确认且稳态识别成功的样本才参与候选参数生成。"));
...
void test_main_source_renders_live_display_frame_for_web() {
    ...
    TEST_ASSERT_NOT_NULL(std::strstr(buffer, "\"/faucet_pulse_traces_v3.bin\""));
