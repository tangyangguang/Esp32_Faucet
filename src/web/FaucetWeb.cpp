...
constexpr std::size_t kSegmentedCalibrationMaxSamples = kCalibrationLongTermSampleSlots;
...
bool appendSegmentedSampleFromTrace(const WaterPulseTrace& trace,
    ...
    const bool copied = copyRamTraceSamples(trace, traceSamples, trace.sampleCount);
    if (!copied) {
        delete[] traceSamples;
        return false;
    }
...
bool activeMeteringSchemeForWeb(MeteringSchemeRecord& output) {
    if (ensureMeteringSchemesReady() && g_context.meteringSchemes->activeScheme(output)) {
        if (meteringSchemeUsedEverForWeb(output)) {
            output.usedEver = true;
        }
        return true;
    }
    if (g_context.app) {
        output = g_context.app->activeMeteringScheme();
        return output.recordUsed && !output.deleted;
    }
    return false;
}
...
bool meteringSchemeUsedEverForWeb(const MeteringSchemeRecord& scheme) {
    ...
    const MeteringSchemeRecord& active = g_context.app->activeMeteringScheme();
    return active.recordUsed && active.id == scheme.id && active.usedEver;
}
...
void sendSegmentedMeteringPanel() {
    ...
    if (activeReady) {
        sendFmt("<div><span>方案标识</span><strong>#%lu</strong><small>修订 <b>rev %lu</b></small></div>",
                static_cast<unsigned long>(activeScheme.id),
                static_cast<unsigned long>(activeScheme.revision));
    } else {
        Esp32BaseWeb::sendChunk("<div><span>方案标识</span><strong>-</strong><small>修订 <b>-</b></small></div>");
    }
...
void sendSchemeDetailButton(const MeteringSchemeRecord& scheme, std::uint32_t activeId) {
    ...
    sendFmt(" data-detail-id='%lu' data-detail-revision='%lu' data-detail-startup-pulses='%luP' "
            ...
            meteringSchemeUsedEverForWeb(scheme) ? "已有出水记录引用" : "暂无出水记录引用");
...
void sendSchemeDetailModal() {
    Esp32BaseWeb::sendChunk("<div id='scheme-detail-modal' class='scheme-detail-modal' aria-hidden='true'>"
                            ...
                            "<tr><th>记录关联</th><td data-scheme-detail-field='useCount'>-</td></tr>"
                            "</table></div></div>");
}
...
void sendActiveMeteringSchemeSummaryPanel() {
    ...
    if (g_context.app) {
        ...
        activeReady = active.recordUsed && !active.deleted;
    }
...
void sendCalibrationParameterPanels() {
    ...
    for (std::size_t i = 0; i < count; ++i) {
        ...
        sendFmt(" <span class='status-pill status-muted'>%s</span>",
                meteringSchemeUsedEverForWeb(scheme) ? "已使用" : "未使用");
...
CalibrationSampleState inspectCalibrationSample(const WaterPulseTrace& trace) {
...
void sendCalibrationSampleStatusPills(const CalibrationSampleState& state, const char* containerClass) {
    ...
    Esp32BaseWeb::sendChunk("<span class='status-pill status-muted'>RAM 明细</span>");
    Esp32BaseWeb::sendChunk("</span>");
}
...
bool readCalibrationTraceFromRequest(WaterPulseTrace& trace, std::uint32_t& traceId) {
    char text[32]{};
    trace = {};
    traceId = 0;
...
bool markCalibrationTraceActual(std::uint32_t traceId,
...
void sendCalibrationSampleRow(const WaterPulseTrace& trace, std::uint32_t samplePulseWindowSec) {
    char startTime[40]{};
    char duration[24]{};
    ...
    const CalibrationSampleState state = inspectCalibrationSample(trace);
    WaterRecordCalibration calibration{};
    ...
    const bool firstWindowReady = firstSecondsPulseTotal(trace, samplePulseWindowSec, firstWindowPulses);
    const char* sourceValue = "ram";
    char rowId[32]{};
    std::snprintf(rowId,
                  sizeof(rowId),
                  "calibrate-%s-%lu",
    ...
    sendTraceDetailLink(true,
                        ...
                        1,
                        "RAM 中",
                        "trace-badge trace-source-link status-muted");
    Esp32BaseWeb::sendChunk("</td><td>");
    sendCalibrationSampleStatusPills(state, "sample-status-pills");
    ...
    if (trialParamsReady) {
        sendMeteringTrialButton("RAM 样本",
                                trialParams,
...
void sendCalibrationSessionAttemptRow(const CalibrationSessionRecord& session,
    ...
    if (hasStoredTrace && samplePulseWindowSec > 0 && stored.trace.sampleCount > 0) {
        WaterPulseTraceSample* traceSamples = new (std::nothrow) WaterPulseTraceSample[stored.trace.sampleCount]{};
        if (traceSamples) {
            const std::size_t copied = g_context.calibrationSessionTraces->readSamples(
                attempt.sessionTraceSlot, traceSamples, stored.trace.sampleCount);
            if (copied == stored.trace.sampleCount) {
                std::size_t count = 0;
                const std::uint32_t endUs = samplePulseWindowSec * 1000000UL;
                while (count < copied && traceSamples[count].elapsedUs < endUs) {
                    ++count;
                }
                firstWindowPulses = effectivePulseCount(stored.trace, traceSamples, count);
    ...
    const bool saved = attempt.status == CalibrationAttemptStatus::Valid &&
                       findLongTermSampleForAttempt(session.sessionId, attempt.attemptIndex, sampleId);
...
void sendLongTermSampleLibraryTable() {
    if (!g_context.calibrationLongTermSamples || !g_context.calibrationLongTermSamples->ready()) {
        const AppStorageStatus status =
            g_context.calibrationLongTermSamples ? g_context.calibrationLongTermSamples->status()
                                                 : AppStorageStatus::Unavailable;
        sendFmt("<p class='hint'>长期样本库不可用：%s（%s）。普通校准仍可使用本次会话样本生成并应用方案。</p>",
                appStorageStatusMessage(status),
                appStorageStatusCode(status));
        Esp32BaseWeb::sendChunk("<table class='long-term-sample-table'><tr><th>时间</th><th>实测容量</th><th>总脉冲</th><th>数据点</th><th>样本状态</th></tr>"
                                "<tr><td colspan='5'>长期样本库暂不可用。</td></tr></table>");
        return;
    }
    ...
    Esp32BaseWeb::sendChunk("<table class='long-term-sample-table'><tr><th>时间</th><th>实测容量</th><th>总脉冲</th><th>数据点</th><th>样本状态</th></tr>");
    if (count == 0) {
        Esp32BaseWeb::sendChunk("<tr><td colspan='5'>还没有长期样本。完成校准会话后，可把优质样本存入这里用于专业生成和复核。</td></tr></table>");
        return;
    }
...
void sendCalibrationGenerationPanel() {
    ...
    if (candidateReady) {
        ...
        char generatedStartupDuration[24]{};
        ...
        formatStartupDurationSeconds(candidate.params.startupDurationMs, generatedStartupDuration, sizeof(generatedStartupDuration));
        ...
        sendFmt("<div class='generated-summary-card'><span>稳态容量计量</span><strong>%luP/L</strong></div>",
                static_cast<unsigned long>(candidate.params.stablePulsePerLiter));
        sendFmt("<div class='generated-summary-card'><span>时间估算</span><strong>%s / %s</strong></div>",
                generatedStartupDuration,
                generatedStableFlow);
        sendFmt("<div class='generated-summary-card'><span>样本覆盖</span><strong>%u 条 / %s</strong></div>",
                static_cast<unsigned>(candidate.sampleCount),
                generatedRange);
        sendFmt("<div class='generated-summary-card'><span>最大误差</span><strong>%s / %u.%u%%</strong></div>",
                generatedError,
...
void formatSensorTemperature(const SensorValue& value, char* out, std::size_t len) {
    ...
    std::snprintf(out,
                  len,
                  "%ld.%02ld C",
                  static_cast<long>(centi / 100),
                  static_cast<long>(std::abs(static_cast<int>(centi % 100))));
}
...
void sendAppCss() {
    ...
    Esp32BaseWeb::sendChunk(".scheme-created-row{background:#fffdf4}.disabled-row{background:#f7f8f8;color:#8a949b}.disabled-row td{color:#8a949b}.disabled-row .status-pill{background:#eef0f0;color:#7b858d}.disabled-row a{color:#6f7a82}"
                            ...
                            "@media(max-width:720px){body{padding:10px}.form-grid,.scheme-edit-grid,.generated-scheme-layout,.trial-estimator-grid,.temperature-calibration-layout,.temperature-calibration-summary,.tds-calibration-summary,.tds-calibration-flow,.tds-step{grid-template-columns:1fr}.span-2,.span-3,.span-4,.span-5,.span-6,.span-8,.span-12,.scheme-span-4,.scheme-span-12{grid-column:1/-1}.usage-grid{grid-template-columns:1fr}.daily-chart svg{min-width:680px}}"
...
void handleRecordDetailPage() {
    ...
    if (!fromCalibration) {
        if (rawRequest) {
            sendPlainTextResponse(404, "普通出水记录不提供脉冲明细。\n");
            return;
        }
    ...
    Esp32BaseWeb::sendChunk("</table><p class='hint'>校准容量统一从校准页脉冲明细保存；计量方案生成只使用长期样本库，不直接扫描 RAM 明细。</p></section>");
...
void handleRecordCalibrationApi() {
    ...
    std::uint32_t traceId = 0;
    if (!readCalibrationTraceFromRequest(trace, traceId)) {
    ...
    if (!markCalibrationTraceActual(traceId, trace.record, actualMl)) {
...
void handleGenerateSegmentedCalibrationApi() {
    ...
    auto ok = [ajax]() {
        if (ajax) {
            ...
            Esp32BaseWeb::redirectSeeOther("/faucet/calibration/flow?saved=generated");
        }
    };
    if (waterTaskActive()) {
        fail(409, "{\"error\":\"busy\"}", "/faucet/calibration/flow?error=busy");
        return;
    }
    if (!generateSegmentedCalibrationResultFromSavedSamples()) {
        fail(400, "{\"error\":\"sample_not_enough\"}", "/faucet/calibration/flow?error=sample_not_enough");
        return;
    }
    ok();
}
...
void handleDiscardGeneratedSchemeApi() {
    ...
    auto ok = [ajax]() {
        if (ajax) {
            ...
            Esp32BaseWeb::redirectSeeOther("/faucet/calibration/flow?saved=generated_discarded");
        }
    };
    if (waterTaskActive()) {
        fail(409, "{\"error\":\"busy\"}", "/faucet/calibration/flow?error=busy");
...
void handleEditMeteringSchemeApi() {
    ...
    if (current.deleted || meteringSchemeUsedEverForWeb(current)) {
...
void handleDeleteMeteringSchemeApi() {
    ...
    std::uint32_t id = 0;
    if (!readMeteringSchemeId(id) ||
        !g_context.meteringSchemes->deleteScheme(id, g_context.nowSeconds ? g_context.nowSeconds() : 0)) {
        Esp32BaseWeb::redirectSeeOther("/faucet/calibration/flow?error=invalid_value");
        return;
    }
    Esp32BaseWeb::redirectSeeOther("/faucet/calibration/flow?saved=scheme_deleted");
}
...
Esp32BaseWeb::Handler handlerFor(const FaucetWebRoute& route) {
    if (route.kind == FaucetWebRouteKind::Page) {
        ...
        if (std::strcmp(route.path, "/faucet/calibration/flow") == 0) {
    ...
    if (std::strcmp(route.path, "/faucet/calibration/flow") == 0) {
        return route.method == FaucetWebMethod::Get ? handleFlowCalibrationPage : handleFlowCalibrationPost;
    }
