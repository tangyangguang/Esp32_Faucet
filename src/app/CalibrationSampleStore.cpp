...
constexpr std::uint8_t kStoreKindLongTerm = 2;
...
bool CalibrationLongTermSampleStore::clear() {
    if (!ready()) {
        return false;
    }
    if (!backend_.exists(path_)) {
    ...
    return writeHeader(backend_, path_, makeHeader(kStoreKindLongTerm, kCalibrationLongTermSampleSlots));
}

bool CalibrationLongTermSampleStore::save(const CalibrationStoredTrace& trace,
                                          const WaterPulseTraceSample* samples,
                                          std::size_t sampleCount,
                                          std::uint32_t& sampleId) {
    sampleId = 0;
    ...
    SampleHeader header{};
    if (!readHeader(backend_, path_, header) || !validHeader(header, kStoreKindLongTerm, kCalibrationLongTermSampleSlots)) {
        return false;
    }
    std::uint8_t freeSlot = kCalibrationLongTermSampleSlots;
    for (std::uint8_t i = 0; i < kCalibrationLongTermSampleSlots; ++i) {
        SampleIndexEntry entry{};
    ...
    if (freeSlot >= kCalibrationLongTermSampleSlots ||
        !writeSamples(backend_, path_, kCalibrationLongTermSampleSlots, freeSlot, samples, sampleCount)) {
        return false;
    }
    CalibrationStoredTrace stored = trace;
    stored.valid = true;
...
bool CalibrationLongTermSampleStore::remove(std::uint32_t sampleId) {
    ...
    for (std::uint8_t i = 0; i < kCalibrationLongTermSampleSlots; ++i) {
        ...
        if (entryUsable(entry) && entry.sampleId == sampleId) {
            return writeBlankEntry(backend_, path_, i);
        }
    }
    return false;
}
...
std::size_t CalibrationLongTermSampleStore::list(CalibrationStoredTrace* output, std::size_t outputCapacity) const {
    ...
    std::size_t count = 0;
    for (std::uint8_t i = 0; i < kCalibrationLongTermSampleSlots && count < outputCapacity; ++i) {
        SampleIndexEntry entry{};
        if (!readEntry(backend_, path_, i, entry)) {
            return count;
        }
        if (entryUsable(entry) && entry.valid != 0) {
            output[count++] = storedFromEntry(entry);
        }
    }
    return count;
}

std::size_t CalibrationLongTermSampleStore::capacity() const {
    return kCalibrationLongTermSampleSlots;
}
...
const char* CalibrationLongTermSampleStore::storageName() const {
    return "calibration-long-term-sample-file";
}
