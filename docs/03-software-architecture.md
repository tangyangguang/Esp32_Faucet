...
- 出水记录放 LittleFS，分页读取，单次最多 200 条；记录目标值、原始脉冲数、过滤脉冲数和当时流量系数。记录本体保持不可变，Web 校准后的实测量、校准类型、参数变化和重校次数写入独立 LittleFS 校准元数据文件。
...
- 流量系数和启动补偿水量放 NVS；可由本地结果页或 Web records 页基于最新可校准出水记录保存，也可在 Esp32Base 系统参数页手动修改。页面单位分别为脉冲/L 和 ml。
...
| 记录 | `/faucet/records` | 按时间范围筛选并分页查看出水记录，显示校准状态和实测量，最新可校准记录可进入校准页保存或重校 |
...
系统配置保存在 `faucet_cfg` namespace，当前版本为 v6。运行统计和滤芯运行量分别保存在 `faucet_stat`、`faucet_run`，不与系统配置版本耦合。

| 版本 | 主要字段 | 加载策略 |
|---|---|---|
| v1 | 单个校准目标 `cal_ml`；滤芯只保存单个寿命天数 `life_d` | 加载后迁移为 v6：忽略 `cal_ml`，滤芯建议/最大天数都继承 `life_d`，新增字段补默认值 |
| v2 | 单个校准目标 `cal_ml`；滤芯已区分建议/最大天数 | 加载后迁移为 v6：忽略 `cal_ml`，保留其他有效字段，新增字段补默认值 |
| v3 | 多个校准候选容量；滤芯建议/最大天数；预设、阀控、OLED 休眠、蜂鸣器等完整参数 | 加载后迁移为 v6：忽略 `cal*_ml`，旧 `oled_s` 迁移为 `displaySleepSec`，新增字段补默认值 |
| v4 | LCD1602 地址、显示休眠、结果显示时间；预设、阀控、滤芯、流量系数等完整参数 | 加载后迁移为 v6：忽略旧校准候选容量，保留其他有效字段，新增字段补默认值 |
| v5 | 删除校准候选容量；records 作为出水记录与校准入口；流量系数保留为高级参数 | 加载后迁移为 v6：新增本地容量/时间调整步进和启动补偿水量 |
| v6 | 新增本地容量/时间调整步进、启动补偿水量；records 作为出水记录与校准入口 | 直接加载并做范围钳位 |
...
- Web 记录页只允许最新一条带原始脉冲且结果为完成、手动停止或暂停超时的记录执行校准；容量目标和时间目标都可校准。
- Web 校准成功后保存记录级校准元数据；记录页用“已校准/未校准”和出水量单元格内的“实测 x.xx L”展示状态，重校会覆盖该记录的实测量并递增重校次数。
- 大容量稳态校准公式为 `newPulsePerMl = pulseCount / actualMl`；小容量记录在当前稳态系数下若表现为少计，则更新启动补偿水量 `startupCompensationMl`，范围 0-300ml。无脉冲时不应用启动补偿，避免无水虚增。
