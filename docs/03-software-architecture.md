...
- 出水记录放 LittleFS 环形文件，分页读取，单次最多 200 条；当前 4MB Flash 分区预算下工程容量固定为 20,000 条，达到容量边界后覆盖最旧记录。记录保存目标值、原始脉冲数、过滤脉冲数、当时启用的计量方案 ID、水温/TDS 摘要和传感器状态标志，不保存 5 个计量参数快照。历史追溯通过方案 ID 查找对应计量方案；计量参数历史固定 20 条，槽位满后覆盖最早的非当前参数，早期历史记录查不到参数时显示已覆盖。记录本体保持不可变，Web 校准后的实测量和重校次数写入独立 LittleFS 校准元数据文件。
