...
3. 校准参数和算法
   ...
   - 普通校准会话确认应用后创建新的历史参数记录并设为当前；专业样本库只提供诊断查看和辅助计算，计算结果可带入手工输入页，保存后成为新的当前参数。
...
- 配置加载只读取当前版本结构；无版本、非当前版本或不可识别版本按当前默认配置加载，显式保存时写入当前结构。流量计计量参数使用独立的历史参数模型，校准结果或手工输入保存后创建新记录并设为当前。方案使用固定 50 槽记录式存储，系统内置 `默认计量参数` 作为兜底参数，启动脉冲数 8P、启动水量 130ml、稳态 248P/L、启动时间 5s、预计稳态流速 1.95L/min。当前参数记录不能删除；非当前记录删除为逻辑删除；槽位满后只复用已逻辑删除槽位。
...
- Web 页面已从安全占位升级为轻量可用页面：状态、预设、记录、统计、滤芯、校准页面已接入；首页用卡片展示启用滤芯寿命进度和今日接水记录；顶层导航只保留“校准”，校准首页作为状态概览，展示当前计量参数、温度状态和水质状态；流量计校准中心通过隐藏路由承载校准流程、样本库、方案生成和方案管理，温度和 TDS 使用校准页的详情视图；页面仍不提供任何远程出水控制入口。
