...
- 出水记录放 LittleFS，分页读取，单次最多 200 条；记录目标值、原始脉冲数、过滤脉冲数、当时启用的计量方案 ID、方案修订号和 3 个核心计量参数快照。记录本体保持不可变，Web 校准后的实测量和重校次数写入独立 LittleFS 校准元数据文件。
...
- 流量计计量方案使用独立持久化域：保存当前启用方案 ID、候选计量方案和动态方案记录。运行时只读取当前方案的启动脉冲数、启动水量、稳态 P/L；候选只能保存为新方案，保存后不自动启用，启用必须是单独动作。已经被出水记录使用过的方案不能删除，只能停用；从未使用过的非当前方案可以删除。
...
| v1-v10 | 旧配置结构；可能包含旧流量字段、预设、阀控、显示、蜂鸣器、滤芯等参数 | 加载后迁移为当前结构：保留可识别用户配置；旧计量字段迁移为当前流量计计量方案；旧候选迁移为候选计量方案；旧滤芯 `life_d` 同时迁移为建议周期和最长周期；旧滤芯运行量迁入 `faucet_run` |
| v11-v12 | 旧多参数配置结构；包含固定编号计量配置和旧候选配置 | 加载后迁移为流量计计量方案模型：当前启用配置迁移为当前方案，非默认有效配置迁移为独立方案，旧候选迁移为候选计量方案 |
...
- Web 校准页生成候选后不直接启用；用户确认后只能保存为新方案。保存新方案不会影响当前计量，用户必须显式启用方案后设备才切换运行参数。
