...
- Web 页面已从安全占位升级为轻量可用页面：状态、配置、预设、日志、统计、滤芯、校准页面已接入；首页显示启用滤芯寿命概览；滤芯列表页只显示查看、状态、设置入口和重置，单个滤芯配置放在独立编辑页；记录页支持分页按钮和每页条数下拉，选项为 20、30、50、100、200，默认 20；校准页显示本地使用说明；页面仍不提供任何远程出水控制入口。
