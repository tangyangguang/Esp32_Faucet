...
- 流量计计量方案使用独立持久化域：保存当前启用方案 ID 和动态方案记录。系统内置 `默认计量方案` 作为首次启动兜底参数，启动脉冲数 8P、启动水量 130ml、稳态 248P/L、启动时间 5s、预计稳态流速 1.95L/min。运行时只读取当前方案的启动脉冲数、启动水量、稳态 P/L；从长期样本库生成的结果只能保存为新方案，保存后不自动启用，启用必须是单独动作。普通校准会话的“确认应用”会创建并启用一套新方案。方案文件固定 50 槽；当前方案不能删除，非当前方案删除为逻辑删除；槽位满后只复用已逻辑删除槽位，早期历史记录查不到方案时显示已覆盖。
