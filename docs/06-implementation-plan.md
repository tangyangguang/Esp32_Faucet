...
- 配置加载只读取当前版本结构；无版本、非当前版本或不可识别版本按当前默认配置加载，显式保存时写入当前结构。流量计计量参数使用独立的流量计计量方案模型，计量方案生成结果只可保存为新方案，启用必须是单独动作。方案使用固定 50 槽记录式存储，系统内置 `默认计量方案` 作为兜底参数，启动脉冲数 8P、启动水量 130ml、稳态 248P/L、启动时间 5s、预计稳态流速 1.95L/min。当前方案不能删除；非当前方案删除为逻辑删除；槽位满后只复用已逻辑删除槽位。
