...
> 2026-06-04 更新：校准功能后续按 `docs/superpowers/specs/2026-06-04-calibration-session-redesign-design.md` 重构为设备级“校准会话”。本文件中关于旧校准页样本列表、RAM 最近明细、已保存明细和计量方案生成的描述只作为当前实现背景；实施新校准能力时以校准会话设计和对应实施计划为准。
...
3. 校准参数和算法
   - 校准重构为设备级校准会话：本地和 Web 共享会话状态，出水动作只允许本地按键执行，Web 只允许进入/退出会话、录入实测容量、放弃样本、生成结果和确认应用。
   - 校准会话最少 2 条有效样本可快速生成，推荐 3 条，最多 5 条有效样本；最多 10 次尝试，包含有效、放弃和系统判定无效的尝试。
   - RAM 只保留最近 1 条完整脉冲明细；本次校准会话样本区使用 Flash 固定 3 槽位并可被新会话覆盖；长期样本库使用 Flash 固定 5 槽位且不自动覆盖。
   - 普通校准会话确认应用后创建并启用新计量方案；专业样本库生成仍保持“保存为新方案，再手动启用”的计量方案管理流程。
...
5. 设备驱动
   - `ValveDriver`：GPIO16，全压吸合后 PWM 保持，当前实现默认全压吸合 5s、保持占空比 70%。业务逻辑、GPIO16 LEDC 适配和主循环接入已完成；5s 是否满足当前电磁阀吸合要求仍待上板实测确认，若实测需要调整必须同步更新代码、测试和文档。
   - `FlowMeter`：GPIO32 中断计数、软件过滤，运行时使用当前流量计计量方案的启动段脉冲/水量和稳态 P/L 分段计量参数。业务逻辑、GPIO32 ISR 缓冲适配和主循环接入已完成，待上板验证。
...
6. Web 页面和 API
   ...
   - `/api/faucet/records` 支持时间范围筛选、分页和每页条数；records 页面只支持最近记录分页，不提供按时间范围筛选。
...
同步日期：2026-06-02；最近上板与构建复核记录见 `docs/07-board-bringup.md`。
...
- 出水结果显示已接入：完成、用户停止、暂停超时、安全停止或流量异常后显示本次结果，默认 30 秒，可在 Web 配置。
- 统计和滤芯运行数据已接入 NVS 持久化：出水任务完成后保存 `faucet_stat` 和 `faucet_run`，避免每个 tick 写入。
- 出水记录已接入 LittleFS 文件环形存储：通过 Esp32Base Fs 按偏移读写 API 写入二进制定长记录，文件不可用时保留 RAM 环形降级写入。当前 4MB Flash 分区预算下工程容量固定为 20,000 条，达到容量边界后覆盖最旧记录。记录包含目标值、原始脉冲数、过滤脉冲数和当时计量方案 ID，不保存完整计量参数快照。记录校准元数据使用独立文件保存实测量、校准类型、参数变化和重校次数，不改写出水记录本体。记录文件固定预分配，并保存主 header 与备份 header；启动时可用备份 header 恢复主 header 写坏导致的异常，避免单次断电拖垮整个记录文件。
...
- Web 路由壳已接入 Esp32Base：21 条页面/API 路由；首页入口统一为 `/index`，滤芯编辑页、记录详情页和校准详情页作为隐藏 HTML 路由注册，不进入导航；已用 native 测试禁止远程出水控制路径。
- Web API 已接入真实状态、预设、记录分页、统计、滤芯和 records 校准动作；滤芯配置 POST 与重置 POST 已接入，重置后写入当前时间、清零累计流量并返回滤芯列表。
- Web 写配置类 API 已通过 Esp32BaseWeb 当前请求方法能力接入 POST 保存：支持配置、单个预设保存、记录校准、校准会话样本入库/长期样本删除、计量方案写入和滤芯重置；空闲状态下保存后立即热更新或写入对应业务存储，出水/确认/暂停期间拒绝修改并返回 busy。
- Web 写配置类 API 采用“复制当前配置 -> 修改临时配置 -> 钳位 -> 保存 -> 热更新”的提交路径；checkbox 未提交会按 false 处理，避免用户关闭蜂鸣器或预设时旧值被保留。
- 配置加载需覆盖旧版本迁移；流量计计量参数改为独立的流量计计量方案模型，计量方案生成结果只可保存为新方案，启用必须是单独动作。方案使用固定 100 槽记录式存储，系统内置 `YF-S201 默认计量方案` 作为兜底参数，启动脉冲数 8P、启动水量 130ml、稳态 248P/L、启动时间 5s、预计稳态流速 1.95L/min。已经被出水记录使用过的方案不能由用户手动删除，只能停用；从未使用过、非当前且不是最后一套有效方案的方案可以删除；槽位满后系统可覆盖最早的非当前方案。旧计量字段迁移为当前计量方案，旧生成结果迁移为计量方案生成结果；只有旧空默认方案时修复为系统内置默认方案；迁移后不继续保存旧字段。检测到未来配置版本时只读加载当前可识别字段并拒绝保存，避免降级固件静默覆盖用户配置。
- 主循环已将流量 ISR 缓冲丢脉冲计数写入状态快照并记录告警日志；开机后检查 LittleFS 剩余空间并在低于 350KB 时告警。
- 主循环已将最大 loop 间隔、业务 tick 最大耗时和 Esp32Base handle 最大耗时写入状态 JSON，用于 Web 重页面访问期间压测 `CANCEL` 响应和自动关阀延迟。
- Web 页面已从安全占位升级为轻量可用页面：状态、预设、记录、统计、滤芯、校准页面已接入；首页用卡片展示启用滤芯寿命进度和今日接水记录；旧校准页同时承载样本校准、计量方案生成和方案管理，后续按校准会话设计拆为“校准”和“计量方案”两个导航入口；页面仍不提供任何远程出水控制入口。
