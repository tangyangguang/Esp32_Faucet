...
- `params`：5 个计量参数。
  ...
  - 启动时长。
  - 预计稳态流速。
- `sourceType`：默认、校准会话生成、长期样本生成、手工、迁移。
...
候选结果不保存完整样本列表，不保存长说明。页面需要展示更细生成过程时，应基于当前样本即时计算。
...
- 计量方案不应追求精确 `useCount`。长期结构只要求 `usedEver`，用于保护已使用方案不被物理删除；展示用使用次数可以不提供，或由 RAM/记录扫描在页面上临时估算并标注“不作为删除依据”。
...
- 计量方案结构应按“运行必需 + 最小生成摘要”重建，不迁就旧的大字段结构。
...
- 计量方案记录不包含样本 traceId 数组、启动阶段统计、长摘要和精确使用次数。
- 长期样本库最多 5 条完整明细。
