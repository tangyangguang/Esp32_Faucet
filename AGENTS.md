...
- `CANCEL` 与其余三个按键统一使用普通按键扫描和 10ms 消抖，不设置专用 ISR、快速通道或绕过消抖逻辑；消抖确认后的停止事件优先关阀，自动关阀动作目标小于 100ms。
- Web 请求不得造成明显的控制卡顿；记录、统计、校准等接口必须分页、小响应或 busy 返回。系统设置显式保存允许同步写入当前配置。
- 重启后默认不继续未完成出水任务。
...
- 除系统设置显式保存外，Web 写配置类 API 必须遵守当前 busy 策略：出水确认、运行、暂停期间不得执行文件扫描、样本聚合、大 JSON 输出或文件写入。系统设置可在任意状态保存；PWM 频率、显示和蜂鸣器立即生效，其余出水参数不得改变当前任务，从下一次任务开始使用。
