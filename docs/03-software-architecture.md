...
- 温度校准详情页输入温度计读数后保存偏移；NTC 的 R25、B 值、上拉标称阻值在系统设置中配置，探头断路/短路直接产生无效值。TDS 校准详情页以 1-5 个校准点作为统一模型，未接导致的 0 不推断为断线。输入电压校准同样统一使用 1-5 点：一点按零偏移比例换算，两点以上线性回归，每点保留 ADC 原始窗口和理论/实际对应关系。
