...
- 流量计使用 ISR 采集最小事件，ISR 内不做复杂计算和日志；四个按键统一由主循环轮询和消抖。
...
- `CANCEL` 与 `OK`、`PLUS`、`MINUS` 使用同一按键扫描和 10ms 消抖路径，不设置专用 ISR、快速通道或原始电平抢占。消抖确认后的 `CancelDown` 优先执行关阀，整体关阀动作目标小于 100ms。
...
| `WaterSensors` | 输入电压、可配置 Beta 模型 NTC 和 TDS AO 的纯算法换算 |
| `WaterSensorManager` | 1s 采样、ADC 故障降级、NTC 断短路识别、输入电压校准、TDS 自动量程与最近 5 个原始采样中位数、实时快照和出水摘要聚合 |
...
| 事件 | 来源 | 说明 |
| --- | --- | --- |
| `KeyCancelDown` | 四键输入 | 经普通消抖确认后停止出水 |
...
- 水温/TDS/输入电压实时采样不写 Flash。TDS 校准只有在用户应用结果时写一次系统配置；会话内可修改参考值并用原始采样重新拟合。输入电压每次增加、修改或删除校准点时，把全部点及重新拟合的比例/修正写入系统配置。出水记录只在出水结束时写一次。
...
| POST | `/faucet/calibration` | 温度校准、TDS 校准、输入电压校准点新增/修改/删除/清空；不提供远程出水/停水 |
...
`WaterRecord` 当前实现为 40 字节并有 `static_assert` 约束。出水期间传感器每 1s 采样一次，单次记录只保存结束前最后 5 个不同采样周期的水温/TDS 均值；主循环重复读取同一快照不得重复计入窗口，样本不足 5 个时按已有样本计算。温度和 TDS 分别通过 `sensorFlags` 标记是否无有效值，避免只启用其中一个传感器时把另一个传感器的默认值当作有效数据。统计聚合按温度/TDS 各自有效性分别计算日均值、最小值和最大值。默认统计排除未校准 TDS 对 TDS 数据的影响，但不丢弃同条记录中有效的温度；API 可按需要包含未校准 TDS 并显示标记。流量校准会话内已保存的量杯实测容量允许修改，修改后从持久化脉冲明细重新分析并生成新候选参数；若旧方案已经应用，不静默覆盖当前方案。
...
所有保存路径都必须先通过 `ConfigStore` 加载配置对象，再合并本次提交字段，通过钳位后提交给 `ConfigStore` 和 `AppController`。AppConfig 和 Web 不得直接写 `faucet_cfg/ver`。系统设置允许在任意状态同步持久化；确认、出水和暂停期间只立即应用 PWM 频率、显示和蜂鸣器，其余控制、计量和传感器参数保存在待应用快照中，当前任务结束后整体应用。结果页不存在活动任务，保存后全部立即应用。

## 校准与计量方案

- 校准首页分为当前计量参数、温度校准、水质校准、输入电压校准四块，只做状态概览和入口；详情通过 `/faucet/calibration?view=...` 切换，不新增路由。
...
- 温度校准详情页输入温度计读数后保存偏移；NTC 的 R25、B 值、上拉标称阻值在系统设置中配置，探头断路/短路直接产生无效值。TDS 实时值直接从 ADS1115 原始码按当前量程恢复模块 AO 电压，并使用最近 5 个原始采样的中位数，避免低电压区整数毫伏截断和单点噪声；量程切换时清空窗口并丢弃切换后的首个样本。TDS 校准详情页以 1-5 个校准点作为统一模型，稳定窗口使用中位数形成校准点，未接导致的 0 不推断为断线；已保存点只允许修改参考 ppm，原始 ppm 和采样电压保持不变并自动重算。输入电压校准同样统一使用 1-5 点：一点按零偏移比例换算，两点以上线性回归，每点保留 ADC 原始窗口和理论/实际对应关系；采集和实时显示统一使用最近 5 次 ADC 原始值的中位数，波动超限时不允许保存；只允许修改万用表实际值，修改后重新拟合。
...
- `CANCEL` 不使用 ISR 或原始电平抢占；与其余按键统一消抖，确认后的停止事件优先关阀。
- 日志文件采用二进制定长记录，Web 输出时转换为 JSON。
- 当前 PCB 不使用 RTC；NTP 未同步时保留启动内相对时间和 boot id，不扫描 DS3231。
- 系统设置页的显式保存会把所有注册字段合并成完整目标配置，并写入当前版本配置；新设备即使尚无 `faucet_cfg/ver`，也不得出现“页面显示已保存、后续任务仍使用默认值”。活动任务保留启动时的控制参数快照，仅 PWM 频率、显示和蜂鸣器按页面说明立即生效。
- 系统设置中的阀门全功率时间、保持占空比和 PWM 频率属于同一“电磁阀”分组。PWM 频率在任意状态保存后立即更新硬件 LEDC，且频率重配不得主动关阀；全功率时间和保持占空比在活动任务结束后随完整配置应用。阀门和蜂鸣器必须使用不同的 LEDC 定时器，避免提示音改写阀门 PWM 频率。
