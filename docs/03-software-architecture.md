...
- 出水记录放 LittleFS 环形文件，分页读取，单次最多 200 条；当前 4MB Flash 分区预算下工程容量固定为 20,000 条，达到容量边界后覆盖最旧记录。记录保存目标值、原始脉冲数、过滤脉冲数、当时启用的计量方案 ID、水温/TDS 摘要和传感器状态标志，不保存 5 个计量参数快照。历史追溯通过方案 ID 查找对应计量方案；方案存储固定 50 槽，槽位满后只复用已逻辑删除槽位，早期历史记录查不到方案时显示已覆盖。记录本体保持不可变，Web 校准后的实测量和重校次数写入独立 LittleFS 校准元数据文件。
...
- 流量计计量参数使用独立持久化域：保存当前参数 ID 和动态历史参数记录。系统内置 `默认计量参数` 作为首次启动兜底参数，启动脉冲数 8P、启动水量 130ml、稳态 248P/L、启动时间 5s、预计稳态流速 1.95L/min。运行时只读取当前参数的启动脉冲数、启动水量、稳态 P/L；校准结果或手工输入保存后创建新的历史参数记录并立即成为当前参数。长期样本库辅助计算只提供临时结果和手工输入预填，不直接保存或启用。历史参数回退通过复制参数进入手工输入并重新保存为最新当前参数。方案文件固定 50 槽；当前参数记录不能删除，非当前记录删除为逻辑删除；槽位满后只复用已逻辑删除槽位，早期历史记录查不到方案时显示已覆盖。
- 校准会话使用独立存储：RAM 只保留当前/最近一次出水的完整脉冲明细；会话样本区为 Flash 固定 3 槽位，新会话开始时清空并覆盖；长期样本库为 Flash 固定 5 槽位，只有用户确认保存/应用后进入，不自动覆盖。校准出水本身仍是正常出水，必须进入出水记录、统计和滤芯累计。
...
- Web records 页支持最近记录分页，不提供按时间范围筛选；按时间范围查询只保留在 `/api/faucet/records`。列表保持紧凑，显示目标、出水量、持续时间、模式、结束原因、脉冲/P-L、计量方案、校准状态和操作。过滤脉冲只在有值时作为脉冲单元格内的辅助诊断信息展示。普通记录页不提供独立脉冲明细保存入口；脉冲明细只通过 RAM 最近明细、校准会话样本区和长期样本库查看；不允许远程打开电磁阀。
...
系统配置保存在 `faucet_cfg` namespace，配置版本由 `ConfigStore` 定义。运行统计和滤芯运行量分别保存在 `faucet_stat`、`faucet_run`，不与系统配置版本耦合。流量计计量方案使用独立持久化域保存，避免每次调整实验参数都整块重写系统配置。用户保存系统配置时按当前结构完整写入，并最后写入当前版本号；保存失败不得更新版本号，避免半写入数据被下一次启动当作完整当前配置读取。出水记录、记录校准元数据、校准样本和计量方案文件都有自己的文件头、版本和校验规则；版本或结构不匹配时返回不可用状态，测试阶段通过人工格式化或重建恢复。
