...
- 出水记录放 LittleFS，分页读取，单次最多 200 条；记录目标值、原始脉冲数、过滤脉冲数和当时启用的稳态 P/L。记录本体保持不可变，Web 校准后的实测量和重校次数写入独立 LittleFS 校准元数据文件。
...
- 脉冲分段计量参数放 NVS：固定 4 个参数槽、当前启用槽位和候选参数。运行时只读取启动脉冲数、启动水量、稳态 P/L；候选参数必须先保存到参数槽，保存到当前启用槽位时立即生效。
...
系统配置保存在 `faucet_cfg` namespace，当前版本为 v11。运行统计和滤芯运行量分别保存在 `faucet_stat`、`faucet_run`，不与系统配置版本耦合。详细规则见 `docs/config-persistence-migration.md`。

| 版本 | 主要字段 | 加载策略 |
|---|---|---|
| 无版本但有可识别字段 | 早期固件或 AppConfig 曾写字段但没有写 `ver`；可能包含旧流量字段、预设、滤芯 `life_d` 等 | 不按空配置处理；按可识别旧字段迁移为 v11，迁移失败保留原数据且不写当前版本 |
| v1-v10 | 旧配置结构；可能包含旧流量字段、预设、阀控、显示、蜂鸣器、滤芯等参数 | 加载后迁移为 v11：保留可识别用户配置；旧计量字段迁移到参数槽 1；旧候选迁移为新候选并把诊断值写入说明；旧滤芯 `life_d` 同时迁移为建议周期和最长周期；旧滤芯运行量迁入 `faucet_run` |
| v11 | 固定 4 个计量参数槽、当前启用槽位、候选参数和现有安全/显示/预设/滤芯配置 | 直接加载并做范围钳位 |
...
- 分段计量公式、参数槽规则和候选保存规则见 `docs/脉冲分段计量参数说明.md` 与 `docs/segmented-metering.md`。
- Web 校准页生成候选后不直接启用；用户必须保存到某个参数槽。保存到非当前启用槽位只覆盖槽位，保存到当前启用槽位会覆盖当前启用参数并立即生效。
