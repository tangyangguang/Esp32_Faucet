...
- 出水记录放 LittleFS 环形文件，分页读取，单次最多 200 条；当前 4MB Flash 分区预算下工程容量固定为 20,000 条，达到容量边界后覆盖最旧记录。记录保存目标值、原始脉冲数、过滤脉冲数、当时启用的计量方案 ID、水温/TDS 摘要和传感器状态标志，不保存 5 个计量参数快照。历史追溯通过方案 ID 查找对应计量方案；方案存储固定 50 槽，已被出水记录引用过的方案在槽位未被环形覆盖前可继续查看，影响计量的修改必须创建新的方案 ID 或不可变版本。记录本体保持不可变，Web 校准后的实测量和重校次数写入独立 LittleFS 校准元数据文件。
...
- 流量计计量方案使用独立持久化域：保存当前启用方案 ID、计量方案生成结果和动态方案记录。系统内置 `默认计量方案` 作为首次启动兜底参数，启动脉冲数 8P、启动水量 130ml、稳态 248P/L、启动时间 5s、预计稳态流速 1.95L/min。运行时只读取当前方案的启动脉冲数、启动水量、稳态 P/L；从长期样本库生成的结果只能保存为新方案，保存后不自动启用，启用必须是单独动作。普通校准会话的“确认应用”会创建并启用一套新方案。方案文件固定 50 槽；用户手动删除只允许删除从未使用过、非当前且不是最后一套有效方案的方案；槽位满后系统可覆盖最早的非当前方案，优先覆盖已停用方案，早期历史记录查不到方案时显示已覆盖。
...
| 流量计校准 | `/faucet/calibration/flow` | 流量校准流程、样本库、方案生成、手工方案管理、测算和显式启用；隐藏路由，不进入顶层导航 |
...
系统配置保存在 `faucet_cfg` namespace，配置版本由 `ConfigStore` 定义。运行统计和滤芯运行量分别保存在 `faucet_stat`、`faucet_run`，不与系统配置版本耦合。流量计计量方案使用独立持久化域保存，避免每次调整实验参数都整块重写系统配置。详细规则见 `docs/config-persistence.md`。
