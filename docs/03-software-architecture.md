...
- 水温/TDS/输入电压实时采样不写 Flash。TDS 校准只有在用户应用结果时写一次系统配置；输入电压每次增加或删除校准点时把全部点及重新拟合的 gain/offset 写入系统配置。出水记录只在出水结束时写一次。
...
| POST | `/faucet/calibration` | 温度校准、TDS 校准、输入电压校准点新增/删除/清空；不提供远程出水/停水 |
...
- 温度校准详情页输入温度计读数后保存偏移；NTC 的 R25、B 值、上拉标称阻值在系统设置中配置，探头断路/短路直接产生无效值。TDS 校准详情页以 1-5 个校准点作为统一模型，未接导致的 0 不推断为断线。输入电压校准同样统一使用 1-5 点：一点按零偏移比例换算，两点以上线性回归，每点保留 ADC 原始窗口和理论/实际对应关系；采集和实时显示统一使用最近 5 次 ADC 原始值的中位数，波动超限时不允许保存。
