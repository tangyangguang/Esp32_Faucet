...
3. 校准参数和算法
   ...
   - 当前本地四键不进入校准向导，Web 支持按量杯实测换算并保存参数，不允许远程打开电磁阀。
...
6. Web 页面和 API
   ...
   - Web 校准允许按量杯实测换算或手动录入系数，不允许打开电磁阀。
...
- Web 页面已从安全占位升级为轻量可用页面：状态、配置、预设、日志、统计、滤芯、校准页面已接入；首页用卡片展示启用滤芯寿命进度；滤芯列表页只显示查看、状态、设置入口和重置，单个滤芯配置放在独立编辑页；记录页支持分页按钮和每页条数下拉，选项为 20、30、50、100、200，默认 20；校准页可按设定出水量和量杯实际水量换算保存流量系数，也可手动配置每升信号数和本地候选容量；页面仍不提供任何远程出水控制入口。
