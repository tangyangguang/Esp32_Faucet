...
- 出水记录放 LittleFS 环形文件，分页读取，单次最多 200 条；当前 4MB Flash 分区预算下工程容量固定为 20,000 条，达到容量边界后覆盖最旧记录。记录保存目标值、有效脉冲数、最小间隔过滤数、当时启用的计量方案 ID、水温/TDS 摘要和传感器状态标志，不保存 5 个计量参数快照。历史追溯通过方案 ID 查找对应计量方案；计量参数历史固定 20 条，槽位满后覆盖最早的非当前参数，早期历史记录查不到参数时显示已覆盖。记录本体保持不可变，Web 校准后的实测量和重校次数写入独立 LittleFS 校准元数据文件。
...
- 水温/TDS 实时采样不写 Flash。TDS 校准只有在用户保存校准结果时写一次系统配置，并递增校准版本；出水记录只在出水结束时写一次。统计仍按出水结束事件更新，不按 1s 采样周期写 Flash。
- 出水记录文件格式随传感器字段升级为当前版本。当前项目仍处于测试阶段，测试记录不做跨结构兼容读取；文件头版本不匹配时应明确返回不兼容，由上板测试阶段手动格式化或重建记录文件。
- 流量计计量参数使用独立持久化域：保存当前参数 ID 和最多 20 条历史参数记录。系统内置 `默认计量参数` 作为首次启动兜底参数，启动脉冲数 8P、启动水量 130ml、稳态 248P/L、启动时间 5s、预计稳态流速 1.95L/min。运行时只读取当前参数的启动脉冲数、启动水量、稳态 P/L；校准结果或手工输入保存后创建新的历史参数记录并立即成为当前参数。历史参数不可编辑、不可删除；需要回退时复制参数进入手工输入并重新保存为最新当前参数。参数文件固定 20 槽；槽位满后覆盖最早的非当前参数，早期历史记录查不到参数时显示已覆盖。
...
- Web records 页支持最近记录分页，不提供按时间范围筛选；按时间范围查询只保留在 `/api/faucet/records`。列表保持紧凑，显示目标、出水量、持续时间、模式、结束原因、脉冲/P-L、计量参数、校准状态和操作。过滤脉冲只在有值时作为脉冲单元格内的辅助诊断信息展示。普通记录页不提供独立脉冲明细保存入口；脉冲明细只通过 RAM 最近明细和校准会话样本区查看；不允许远程打开电磁阀。
...
| 记录 | `/faucet/records` | 分页查看最近出水记录，显示校准状态、实测量和普通详情入口 |
| 记录详情 | `/faucet/records/detail` | 单条普通记录详情；隐藏路由，不进入导航 |
| 校准 | `/faucet/calibration` | 校准首页，概览当前计量参数、温度状态和水质状态，并进入各校准详情 |
| 流量计校准 | `/faucet/calibration/flow` | 流量校准流程、本次会话样本、推荐参数、手工输入参数、历史参数和测算；隐藏路由，不进入顶层导航 |
...
struct WaterRecord {
    ...
    int16_t temperatureAvgCentiC;
    int16_t temperatureMinCentiC;
    int16_t temperatureMaxCentiC;
    uint16_t tdsAvgPpm;
    uint16_t tdsMinPpm;
    uint16_t tdsMaxPpm;
    uint16_t tdsVoltageAvgMv;
    uint16_t sensorSampleCount;
    uint16_t sensorFlags;
    uint16_t tdsCalibrationRevisionAtRun;
    uint8_t tdsCalibrationModeAtRun;
    uint8_t tdsCalibratedAtRun;
    uint8_t tdsTemperatureCompensatedAtRun;
    uint8_t tdsTempFallback25CAtRun;
    uint8_t sensorReserved[4];
...
`WaterRecord` 当前实现为 64 字节并有 `static_assert` 约束。传感器统计聚合规则为：每次出水记录平均值、最小值、最大值和有效采样数；统计页按记录的有效采样数加权计算日均值。默认统计排除未校准 TDS 或传感器异常记录，API 可按需要包含未校准数据并显示标记。

## 持久化配置版本化

系统配置保存在 `faucet_cfg` namespace，配置版本由 `ConfigStore` 定义。运行统计和滤芯运行量分别保存在 `faucet_stat`、`faucet_run`，不与系统配置版本耦合。流量计计量参数使用独立持久化域保存，避免每次调整实验参数都整块重写系统配置。用户保存系统配置时按当前结构完整写入，并最后写入当前版本号；保存失败不得更新版本号，避免半写入数据被下一次启动当作完整当前配置读取。出水记录、记录校准元数据、校准会话样本和计量参数文件都有自己的文件头、版本和校验规则；版本或结构不匹配时返回不可用状态，测试阶段通过人工格式化或重建恢复。
...
| 不可识别版本 | 版本号小于 0 或无法识别 | 使用默认配置并记录警告；用户可通过 Web 系统工具重新生成配置 |
...
- 流量计校准中心通过 `/faucet/calibration/flow` 进入，页面内同时展示校准流程、当前会话样本、推荐参数、历史参数、手工输入和测算工具；进入页面不会开始出水。
- 校准会话最少 2 条有效样本可生成，推荐 3 条，有效样本最多 6 条，尝试最多 6 次；实际出水/停水只允许本地按键执行，Web 只录入实测容量、移除样本、保存推荐参数或手工创建当前参数。
...
- Web 校准页生成候选参数后不提供单独“保存方案/切换参数”流程；校准会话保存结果或手工输入保存后直接成为最新当前参数。历史参数只用于查看和复制，复制后必须重新保存为新的当前参数。
