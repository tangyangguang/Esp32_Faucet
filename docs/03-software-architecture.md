...
系统配置保存在 `faucet_cfg` namespace，当前版本为 v11。运行统计和滤芯运行量分别保存在 `faucet_stat`、`faucet_run`，不与系统配置版本耦合。

| 版本 | 主要字段 | 加载策略 |
|---|---|---|
| v1-v10 | 旧配置结构；可能包含旧流量字段、预设、阀控、显示、蜂鸣器、滤芯等参数 | 加载后迁移为 v11：保留可识别用户配置；旧计量字段迁移到参数槽 1；旧候选迁移为新候选并把诊断值写入说明；迁移后只保存新字段 |
