...
- 配置加载已覆盖 v1/v2/v3/v4/v5 到 v6 的迁移；v6 新增本地容量/时间调整步进和启动补偿水量，旧 `cal_ml` 和 `cal*_ml` 加载时忽略，旧 `oled_s` 会迁移为 `displaySleepSec`；检测到未来配置版本时只读加载当前字段并拒绝保存，避免降级固件静默覆盖用户配置。
- 主循环已将流量 ISR 缓冲丢脉冲计数写入状态快照并记录告警日志；开机后检查 LittleFS 剩余空间并在低于 100KB 时告警。
- Web 页面已从安全占位升级为轻量可用页面：状态、预设、记录、统计、滤芯页面已接入；首页用卡片展示启用滤芯寿命进度；滤芯列表页只显示查看、状态、设置入口和重置，单个滤芯配置放在独立编辑页；记录页支持时间范围筛选、分页按钮和每页条数下拉，选项为 20、30、50、100、200，默认 20；记录表不再使用“诊断”列，改为脉冲/P-L、校准状态和校准/重校操作，过滤脉冲只在有值时作为小字显示；页面仍不提供任何远程出水控制入口。
