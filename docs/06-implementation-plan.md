...
3. 校准参数和算法
   ...
   - 校准会话最少 2 条有效样本可快速生成，推荐 3 条，最多 3 条有效样本；最多 6 次尝试，包含有效、放弃和系统判定无效的尝试。
...
6. Web 页面和 API
   ...
   - 注册查询和配置 API：状态、预设、记录、统计、滤芯，以及记录校准、脉冲明细和计量方案管理动作。
...
- Web 路由壳已接入 Esp32Base：页面/API 路由已按业务边界注册；首页入口统一为 `/index`，滤芯编辑页、记录详情页、校准详情页和流量计校准中心作为隐藏 HTML 路由注册，不进入顶层导航；已用 native 测试禁止远程出水控制路径。
- Web API 已接入真实状态、预设、记录分页、统计、滤芯和 records 校准动作；滤芯配置 POST 与重置 POST 已接入，重置后写入当前时间、清零累计流量并返回滤芯列表。
- Web 写配置类 API 已通过 Esp32BaseWeb 当前请求方法能力接入 POST 保存：支持配置、单个预设保存、记录校准、流量校准会话样本入库/长期样本删除、计量方案写入、温度校准、TDS 校准和滤芯重置；空闲状态下保存后立即热更新或写入对应业务存储，出水/确认/暂停期间拒绝修改并返回 busy。
- Web 写配置类 API 采用“复制当前配置 -> 修改临时配置 -> 钳位 -> 保存 -> 热更新”的提交路径；checkbox 未提交会按 false 处理，避免用户关闭蜂鸣器或预设时旧值被保留。
- 配置加载只读取当前版本结构；无版本、非当前版本或不可识别版本按当前默认配置加载，显式保存时写入当前结构。流量计计量参数使用独立的流量计计量方案模型，计量方案生成结果只可保存为新方案，启用必须是单独动作。方案使用固定 50 槽记录式存储，系统内置 `默认计量方案` 作为兜底参数，启动脉冲数 8P、启动水量 130ml、稳态 248P/L、启动时间 5s、预计稳态流速 1.95L/min。已经被出水记录使用过的方案不能由用户手动删除，只能停用；从未使用过、非当前且不是最后一套有效方案的方案可以删除；槽位满后系统可覆盖最早的非当前方案。
...
- Web 页面已从安全占位升级为轻量可用页面：状态、预设、记录、统计、滤芯、校准页面已接入；首页用卡片展示启用滤芯寿命进度和今日接水记录；顶层导航只保留“校准”，校准首页分为流量计校准、温度校准、水质校准三块，流量计校准中心通过隐藏路由承载校准流程、样本库、方案生成和方案管理；页面仍不提供任何远程出水控制入口。
