...
3. 本地流量校准
   ...
   - 支持 Web 配置最多 4 个本地校准候选容量，默认 1.50L/7.50L/0/0；容量为 0 表示停用。
   - 支持至少 2 次采样、5% 偏差检查、确认保存。
   - 校准采样不写出水日志，不更新统计和滤芯。
...
- `main.cpp` 已接入本地控水主循环：配置加载、三键读取、`STOP` ISR 快速停止、流量 ISR 缓冲、业务 tick、电磁阀 PWM 输出、蜂鸣器事件输出、RTC 自动检测。
- 本地流量校准已接入：长按 `OK` 进入，`NEXT` 在 Web 启用的候选容量中选择，`OK` 二次确认启动采样，达到容器刻度后 `OK` 结束采样，至少两次一致后 `OK` 保存；校准不写入出水日志、统计或滤芯用量。
...
- Web 写配置类 API 已通过 Esp32BaseWeb 当前请求方法能力接入 POST 保存：支持配置、单个预设、校准参数保存；空闲状态下保存后立即热更新到运行中控制器、流量计、电磁阀、OLED 和蜂鸣器，出水/确认/暂停/校准采样期间拒绝修改并返回 busy。
- Web 写配置类 API 采用“复制当前配置 -> 修改候选配置 -> 钳位 -> 保存 -> 热更新”的提交路径；checkbox 未提交会按 false 处理，避免用户关闭蜂鸣器或预设时旧值被保留。
- 配置加载已覆盖 v1/v2 到 v3 的迁移；检测到未来配置版本时只读加载当前字段并拒绝保存，避免降级固件静默覆盖用户配置。
